...
if(!empty($id)){
    $qry = $conn->query("SELECT * FROM lessons WHERE id = '$id'");
...
<div class="card card-outline cardprimary w-fluid">
	<div class="card-header">
		<h3 class="card-title"><?php echo $rw['title']; ?></h3>
        <div class="card-tools">
            <button class="btn btn-sm btn-flat btn-primary" type="button">My Course</button>
        </div>
	</div>
	<div class="card-body">
        <div class="w-100">
            <div class="col-md-12">
                <span class="truncate float-right" style="max-width:calc(50%);font-size:13px !important;font-weight:bold">Course: 
                <?php
                    $id = $rw['subject_id']; 
        ...
        echo (preg_replace("/(\[pdf_view\spath\s=\s+)([a-zA-Z0-9\/.]+)(\])/",$pdf_pattern,html_entity_decode($rw['description'])));
         ?>
        </div>
        <hr>
        <div class="w-100">
            <div class="col-md-12">
            <span class="float-right"><b>Prepared By: </b><?php
                    $id = $rw['faculty_id']; 
